package backend.alex.store;

import backend.alex.io.InputStream;
import backend.alex.io.OutputStream;
import backend.alex.utils.Utils;
import backend.alex.util.crc32.CRC32HGenerator;
import backend.alex.util.whirlpool.Whirlpool;
...
public final class Index {
    ...
    public boolean removeFile(int archiveId, int fileId, int compression, int[] keys) {
        if (!this.fileExists(archiveId, fileId)) {
            ...
            reference.setWhirlpool(Whirlpool.getHash(var141, 0, var141.length - 2));
    ...
    public boolean putFile(int archiveId, int fileId, int compression, byte[] data, int[] keys, boolean rewriteTable, boolean resetCache, int archiveName, int fileName) {
        ...
        reference.setWhirlpool(Whirlpool.getHash(var191, 0, var191.length - 2));
        if (archiveName != -1) {
        ...
        if (!this.archiveExists(archiveId)) {
            ...
            if (archive == null) {
                ...
                if (reference.getRevision() != archive.getRevision()) {
                    ...
                    reference.setWhirlpool(Whirlpool.getHash(closedArchive, 0, closedArchive.length - 2));
