...
import content.global.skill.cooking.brewing.CalquatKegDecant
...
class DecantPlugin : InteractionListener {

    companion object {
        val potions = Consumables.potions.toIntArray()
        val decantableItems = potions + CalquatKegDecant.allKegIds
    }
    ...
    override fun defineListeners() {
        on(IntType.NPC, "decant") { player, node ->
            ...
            for (item in potionRemove) if (!removeItem(player, item)) {
                return@on false
            }
            for (item in potionAdd) addItem(player, item.id, item.amount)

            // Decant kegs
            val (kegRemove, kegAdd) = CalquatKegDecant.decantCalquat(player.inventory)
            for (item in kegRemove) if (!removeItem(player, item)) {
                return@on false
            }
            for (item in kegAdd) addItem(player, item.id, item.amount)

            player.dialogueInterpreter.open(DecantingDialogue(), node.asNpc())
            return@on true
        }

        onUseWith(IntType.ITEM, decantableItems, *decantableItems) { player, used, with ->
            ...
            if (CalquatKegDecant.isKeg(used.id) || CalquatKegDecant.isKeg(with.id)) {
                return@onUseWith CalquatKegDecant.combineKegs(player, used, with)
            }
        ...
        onUseWith(IntType.ITEM, Items.BEER_GLASS_1919, *CalquatKegDecant.allKegIds) { player, used, with ->
            ...
            return@onUseWith CalquatKegDecant.pourIntoGlass(player, glass, keg)
        }
        ...
        onUseWith(IntType.ITEM, potions, Items.VIAL_229) { player, used, with ->
            ...
            if (!player.savedData.activityData.barbarianHerblore) {
                return@onUseWith false
            }
