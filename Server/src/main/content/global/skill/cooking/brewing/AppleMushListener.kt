...
class AppleMushListener : InteractionListener {
    ...
    private data class BarrelLocations(val start: Location, val jump: Location, val tap: Location, val stand: Location)

    private val barrelLocations = mapOf(
        // Region 'Port Phasmatys'
        14747 to BarrelLocations(
        ...
        // Region 'Tree Gnome Village'?
        9780 to BarrelLocations(
    ...
    private val emptyAppleBarrels = intArrayOf(
        Objects.APPLE_BARREL_7403,
        Objects.APPLE_BARREL_8807
    )

    private val fullAppleBarrels = intArrayOf(
        Objects.APPLE_BARREL_7404,
        Objects.APPLE_BARREL_8808
    )

    private val appleTaps = intArrayOf(
        Objects.BARREL_TAP_7406,
        Objects.BARREL_TAP_8737
    )

    private val MUSH_ANIM = Animation(Animations.JUMPING_ON_APPLE_MUSH_2306)
        ...
        queueScript(player, 1 ,QueueStrength.SOFT) { counter ->
            when (counter) {
                ...
                2 -> {
                    ...
                    if (barrel.id == Objects.APPLE_BARREL_7403) sendGraphics(414, barrelLoc)
                        else sendGraphics(Graphics.BIG_ROTTEN_APPLE_BARREL_413, barrelLoc)
                    swapBarrel(barrel)
