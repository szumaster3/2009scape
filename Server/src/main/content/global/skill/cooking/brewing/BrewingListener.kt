...
const val KeladgrimRegion = 11679
const val PortPhasmatysRegion = 14747

val regionToVarbitMap = mapOf(
    PortPhasmatysRegion to BrewingVat.PORT_PHAS,
    KeladgrimRegion to BrewingVat.KELDAGRIM
)

class BrewingListener : InteractionListener {

    private val aleIngredientsId = BrewProduct.getIngredients()

    private val vatsNeedingYeast = intArrayOf(
        Scenery.FERMENTING_VAT_7444,
        Scenery.FERMENTING_VAT_7449,
        Scenery.FERMENTING_VAT_7454,
        Scenery.FERMENTING_VAT_7459,
        Scenery.FERMENTING_VAT_7464,
        Scenery.FERMENTING_VAT_7469,
        Scenery.FERMENTING_VAT_7474,
        Scenery.FERMENTING_VAT_7479,
        Scenery.FERMENTING_VAT_7484,
        Scenery.FERMENTING_VAT_7489,
        Scenery.FERMENTING_VAT_8871
    )
    ...
    override fun defineListeners() {
        ...
        /*
         * Handles filling an empty fermenting vat with water using buckets.
         */
        ...
        onUseWith(SCENERY, Items.BUCKET_OF_WATER_1929, Scenery.FERMENTING_VAT_7437) { player, _, _ ->
            if (removeItem(player, Item(Items.BUCKET_OF_WATER_1929, 2))) {
                queueScript(player, 0) {
                    ...
                    val vatVarbit = regionToVarbitMap.getValue(player.location.regionId).varbit
        ...
        /*
         * Handles filling an empty vat with apple mush.
         */
        ...
        /*
         * Handles adding barley malt to a water-filled fermenting vat.
         */
        ...
        onUseWith(SCENERY, Items.BARLEY_MALT_6008, Scenery.FERMENTING_VAT_7438) { player, _, _ ->
            if (inInventory(player, Items.BARLEY_MALT_6008, 2)) {
                queueScript(player, 0, QueueStrength.SOFT) { counter ->
                    when (counter) {
                        0 -> {
                            if (!removeItem(player, Item(Items.BARLEY_MALT_6008, 1))) {
                                return@queueScript stopExecuting(player)
                            }
        ...
        /*
         * Handles adding "The Stuff" to a partially prepared fermenting vat.
         */
        ...
        /*
         * Handles adding brewing ingredients to fermenting vats.
         */
        ...
        onUseWith(SCENERY, aleIngredientsId, Scenery.FERMENTING_VAT_7441) { player, used, _ ->
            ...
            if (used.id == Items.KELDA_HOPS_6113 && player.location.regionId != KeladgrimRegion) {
        ...
        /*
         * Handles adding yeast to fermenting vats that are ready for brewing.
         */
        ...
        onUseWith(SCENERY, Items.ALE_YEAST_5767, *vatsNeedingYeast) { player, _, with ->
            ...
            if (inInventory(player, Items.ALE_YEAST_5767, 1)) {
                ...
                if (with.id == Scenery.FERMENTING_VAT_8871 &&
                    player.location.regionId == KeladgrimRegion &&
                    getVarbit(player, 736) == 68
                ) {
        ...
        /*
         * Handles turning the valve to complete fermentation and fill barrels.
         */

        on(intArrayOf(Scenery.VALVE_7442, Scenery.VALVE_7443), SCENERY, "turn") { player, _ ->

            when {
                player.location.regionId == KeladgrimRegion && KeldaBypasses.keldaIsBrewing(player) ->
                    sendMessage(player, "The contents of the vat haven't finished fermenting yet.")

                player.location.regionId == KeladgrimRegion && KeldaBypasses.keldaIsComplete(player) ->
        ...
        /*
         * Handles draining a barrel that contains bad ale
         */
        ...
        /*
         * Handles leveling a barrel using a drink container.
         */

        on(barrels, SCENERY, "Level") { player, _ ->
            queueScript(player, 1) {

                val goAgain: Boolean = when {
                    inInventory(player, Items.BEER_GLASS_1919) ->
                        getVat(player).levelBarrel(Items.BEER_GLASS_1919)

                    inInventory(player, Items.CALQUAT_KEG_5769) ->
                        getVat(player).levelBarrel(Items.CALQUAT_KEG_5769)
                ...
                if (goAgain) delayScript(player, 1) else stopExecuting(player)
            }
            return@on true
        }

        /*
         * Handles leveling a barrel using a container on it.
         */

        onUseWith(
            SCENERY,
            intArrayOf(Items.BEER_GLASS_1919, Items.CALQUAT_KEG_5769),
            *barrels
        ) { player, used, _ ->
    ...
    private fun getVat(player: Player): FermentingVat {
        return regionToVarbitMap.getValue(player.location.regionId).getVat(player)
    }
}