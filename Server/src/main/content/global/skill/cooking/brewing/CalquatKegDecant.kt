...
object CalquatKegDecant {
    ...
    val calquatDrinks =
        BrewProduct.values()
            .filter { it.productID.size == 4 }
            .associateWith { CalquatKegs (
                fourDose = it.productID[1],
                glass = it.productID[0],
                fourMature = it.productID[3],
                matureGlass = it.productID[2]
            )
        }
    ...
    // Reverse lookup map
    private val kegLookupMap: Map<Int, Pair<BrewProduct, CalquatKegs>> by lazy {
    ...
    fun getKeg(itemId: Int): Pair<BrewProduct, CalquatKegs>? = kegLookupMap[itemId]
    ...
    fun decantCalquat(container: core.game.container.Container): Pair<List<Item>, List<Item>> {
        val doseCount = HashMap<Pair<BrewProduct, Boolean>, Int>() // (BrewProduct, isMature), Total Doses
