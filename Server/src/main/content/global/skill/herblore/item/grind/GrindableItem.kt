 ...
 */
enum class GrindableItem(val items: Set<Int>, val product: Int, val message: String) {
    ...
    companion object {
        private val map = GrindableItem.values().flatMap { g -> g.items.map { it to g } }.toMap()
        fun forID(id: Int): GrindableItem? = map[id]
    }
}