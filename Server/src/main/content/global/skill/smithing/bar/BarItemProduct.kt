...
/**
 * Represents a smithing product that can be created from a specific bar type.
 */
enum class BarItemProduct(
    val barType: BarType,
    ...
    companion object {
        /**
         * Maps product item ids to their [BarItemProduct].
         */
        private val barItemProduct: MutableMap<Short, BarItemProduct> = HashMap()

        /**
         * @return the [BarItemProduct] for the given product item id, or `null` if not found.
         */
        fun forId(item: Int): BarItemProduct? = values().find { it.product == item }
        ...
        init {
            for (bar in values()) {
                barItemProduct[bar.product.toShort()] = bar
            }
        }

        /**
         * @return all [BarItemProduct] of the specified [BarType].
         */
        fun getBars(type: BarType): Array<BarItemProduct?> {
