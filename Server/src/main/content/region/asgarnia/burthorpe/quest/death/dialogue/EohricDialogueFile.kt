    ...
    override fun handle(componentID: Int, buttonID: Int) {
        when (getQuestStage(player!!, Quests.DEATH_PLATEAU)) {
            in 5..9 -> when (stage) {
                ...
                10 -> npcl(FaceAnim.FRIENDLY, "There was only one guard on last night. Harold. He's a nice lad, if a little dim.").also { stage++ }
                ...
                20 -> npcl(FaceAnim.FRIENDLY, "No, sorry. I wouldn't want to go north-east from here, it's very rocky and barren.").also { stage = END_DIALOGUE }
            }
            10 -> when (stage) {
                0 -> player(FaceAnim.FRIENDLY, "Hi!").also { stage++ }
                1 -> npc(FaceAnim.FRIENDLY, "Hi, can I help?").also { stage++ }
                2 -> showTopics(
                    ...
                    Topic(FaceAnim.FRIENDLY, "Do you know of another way up Death Plateau?", 20),
                    Topic(FaceAnim.FRIENDLY, "No, I'm just looking around.", END_DIALOGUE),)
                10 -> npc(FaceAnim.FRIENDLY, "Harold is staying at the Toad and Chicken.").also { stage = END_DIALOGUE }
                20 -> npcl(FaceAnim.FRIENDLY, "No, sorry. I wouldn't want to go north-east from here, it's very rocky and barren.").also { stage = END_DIALOGUE }
            }
            11 -> when (stage) {
                0 -> player(FaceAnim.FRIENDLY, "Hi!").also { stage++ }
                ...
                3 -> npcl(FaceAnim.THINKING, "Hmm. Harold has got in trouble a few over his drinking and gambling. Perhaps he'd open up after a drink?").also { stage++ }
            ...
            12 -> when (stage) {
                ...
                3 -> npcl(FaceAnim.THINKING, "Yes, if you buy Harold a beer he might talk to you. I also know he has a weakness for gambling. Hope that helps!").also { stage++ }
