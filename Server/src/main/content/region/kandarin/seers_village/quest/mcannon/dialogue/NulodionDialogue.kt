    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (quest!!.getStage(player)) {
            70 ->
                when (stage) {
                    ...
                    7 -> {
                        ...
                        player.inventory.add(content.region.kandarin.seers_village.quest.mcannon.DwarfCannon.NULODION_NOTES, player)
                        player.inventory.add(content.region.kandarin.seers_village.quest.mcannon.DwarfCannon.MOULD, player)
                        end()
                    }
                }
            ...
            80 ->
                when (stage) {
                    0 -> {
                        if (!player.hasItem(content.region.kandarin.seers_village.quest.mcannon.DwarfCannon.NULODION_NOTES)) {
                            player("I've lost the notes.")
                            stage = 100
                        }
                        if (!player.hasItem(content.region.kandarin.seers_village.quest.mcannon.DwarfCannon.MOULD)) {
