...
class LuthasDialogue(player: Player? = null) : Dialogue(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            10 -> npc(FaceAnim.HAPPY, "Yes, I can sort something out. There's a crate ready to", "be loaded onto the ship.").also { stage++ }
            11 -> npc(FaceAnim.NEUTRAL, "You wouldn't believe the demand for bananas from", "Wydin's shop over in Port Sarim. I think this is the", "third crate I've shipped him this month..").also { stage++ }
            12 -> npc(FaceAnim.HAPPY, "If you could go fill it up with bananas, I'll pay you 30", "gold.").also { stage++ }
            13 -> {
                end()
                player.getSavedData().globalData.setLuthasTask(true)
            }
            14 -> npc(FaceAnim.NEUTRAL, "Well I know her pretty well. She doesn't cause me any", "trouble any more").also { stage++ }
            15 -> npc(FaceAnim.NEUTRAL, "She doesn't even search my export crates anymore.", "She knows they only contain bananas.").also { stage++ }
            16 -> player(FaceAnim.SUSPICIOUS, "Really? How interesting. Whereabouts do you send", "those to?").also { stage++ }
            17 -> npc(FaceAnim.NEUTRAL, "There is a little shop over in Port Sarim that buys", "them up by the crate. I believe it is run by a man", "called Wydin.").also { stage = END_DIALOGUE }
            18 -> {
