...
class ScriptAPI(private val bot: Player) {
    ...
    fun List<Scenery?>.toNodeList(): List<Node> = this.filterNotNull().map { it as Node }
    ...
    fun distance(n1: Node, n2: Node): Double {
        return sqrt(
            (n1.location.x - n2.location.x.toDouble()).pow(2.0) + (n2.location.y - n1.location.y.toDouble()).pow(2.0)
        )
    }
    ...
    fun useWith(bot: Player, itemId: Int, node: Node?) {
        ...
        val event = if (flipped) NodeUsageEvent(bot, 0, node, item) else NodeUsageEvent(bot, 0, item, childNode ?: node)
    ...
    fun getNearestNodeFromList(acceptedNames: List<String>, isObject: Boolean): Node? {
        val region = RegionManager.forId(bot.location.getRegionId()).planes[bot.location.z]
        return if (isObject)
            processEvaluationList(
                region.objectList.filterNotNull().map { it as Node },
    ...
    fun getNearestNode(id: Int, isObject: Boolean): Node? {
        val region = RegionManager.forId(bot.location.regionId).planes[bot.location.z]
        return if (isObject)
            processEvaluationList(
                region.objectList.toNodeList(),
                acceptedId = id
            )
        else
            processEvaluationList(
    ...
    fun evaluateViability(
        ...
        if (acceptedPredicate != null) {
            ...
            val name = e?.name
            return (acceptedNames?.stream()?.anyMatch({ s -> s.equals(name, true) }) ?: true &&
                    !Pathfinder.find(bot, e).isMoveNear)
        }
    }
    ...
    fun getNearestGameObject(loc: Location, objectId: Int): Scenery? {
        return RegionManager.forId(loc.regionId).planes[0].objects
            ?.flatten()
            ?.filterNotNull()
            ?.filter { it.id == objectId }
            ?.minByOrNull { distance(loc, it) }
    }
                ...
                if (canSell && saleIsBigNews(actualId, itemAmt)) {
                    Repository.sendNews(
                        SERVER_GE_NAME +
                                ...
                                itemAmt +
                                " " +
                                ItemDefinition.forId(actualId).name.toLowerCase() +
                                " on the GE."
                    )
                }
                            ...
                            1521 -> continue
                                ...
                                sendNews(
                                    SERVER_GE_NAME +
                                            ...
                                            itemAmt +
                                            " " +
                                            ItemDefinition.forId(actualId).name.lowercase() +
                                            " on the GE."
                                )
                        }
                    }
            ...
            override fun pulse(): Boolean {
                ...
                for (item in bot.inventory.toArray()) {
                    ...
                    when (item.id) {
                        ...
                        Items.COINS_995 -> continue
                    }
                    bot.bank.add(item)
                    bot.inventory.remove(item)
                }
                //                log(this::class.java, Log.FINE, "${bot.username} Just finished
                // banking at ${bot.location} || Bank contents: ${bot.bank}")
                return true
            }
        }
        if (bank != null) {
    ...
    /**
     * Function to determine whether or not to bother everyone on the server with the big news that a bot is selling
     * something to the GE, based on item value
     *
     * @param itemID
     * @param amount
     * @author Gexja
    ...
    fun saleIsBigNews(itemID: Int, amount: Int): Boolean {
        return ItemDefinition.forId(itemID).getAlchemyValue(true) * amount >=
                (GameWorld.settings?.ge_announcement_limit ?: 500)
    }
    ...
    fun eat(foodId: Int) {
        ...
        if (
            bot.skills.getStaticLevel(Skills.HITPOINTS) * RandomFunction.random(0.5, 0.75) >= bot.skills.lifepoints &&
    ...
    /**
     * Method to equip list of items and set stats Useful for starting a bot up
     *
    ...
    /**
     * Method to equip a single item and set stats Useful for starting a bot up
     *
    ...
    fun equipAndSetStats(item: Item) {
        ...
        if (reqs != null)
            for (req in configs["requirements"] as HashMap<Int, Int>) bot.skills.setStaticLevel(req.key, req.value)
        bot.skills.updateCombatLevel()
    }

    /**
     * Method to load appearance and equipment from JSON Useful for starting a bot up
     *
